...
module.exports = class {
  ...
  async run (message) {
    ...
    const language = (guildSettings && guildSettings.lang) || 'en-US'
        ...
        if (f) {
          ...
          if (message.content.match(mentionHelp)) {
            const helpEmbed = new Discord.MessageEmbed()
                .setTitle("Shiber - One of Best Moderation Bots")
                .setDescription(`
            Looking for some peace? Can't find trustworthy moderators? Need round-the-clock moderation for your server?
    Shiber. Introducing the new, easy to configure smart moderation bot...
    Shiber is a moderation bot with a wide range of commands and features that will not only let you have peace of mind, but will let your users feel safer, thanks to Shiber's Auto-moderation features, and its proprietary Banlist.
    First, Auto-moderator can achieve what normal, human moderators would do. From preventing spam, to deleting NSFW (Not Safe For Work) content, Shiber has an increasing amount of Auto-moderator features that will not only protect your server, but minimize the time wasted on moderating, and maximizing the happy time spent on chatting with others! At the time of writing, Shiber's available Auto-moderator options are: Anti Bad Words, Anti @Everyone, Anti Discord Invites, Anti Links, Anti NSFW, Anti Spam, Max Message Lines, Max Message Mentions.
    Second, thanks to its proprietary Banlist, Shiber can prevent known abusers from joining your server, or just warn you if they do. You can even scan your server for them using the "scan" command. Lastly, you can report abusive users yourself by using the "report" command.
    Third, Shiber can achieve the basic moderation commands like nicking, clearing messages, punishing, muting, kicking, soft banning and banning. But that's not all what's in there! Shiber has two unique commands specially desgined to prevent all users (exluding Moderators and Administrators) from sending messages in a specific channel ("lockit" command), or in the whole server ("lockdown" command).

    Dashboard: [https://shiber.xyz](https://shiber.xyz)
    Invite Shiber: [https://shiber.xyz/invite](https://shiber.xyz/invite)
    Support Server: [https://shiber.xyz/discord](https://shiber.xyz/discord)

    Basic Commands:
    \`${guildSettings.prefix}help\` - Get a list of commands.
    \`${guildSettings.prefix}info\` - Basic information about Shiber.

    Created by MrDioogo#0001
            `)
                .setColor("BLUE")
                .setTimestamp();
          ...
          if (cmd && usrs.includes(message.author.id)) return reply("Seems like you can't acces this. You've been banned by one of bot admins for more details regarding your case and appeal info, join our support server https://shiber.xyz/discord.");
          ...
          if (cmd.conf.enabled === false) return reply("This command is currently globally disabled. Visit our dashboard at https://shiber.xyz/.");
    ...
    `;
          ...
          try {
            await cmd.run({message, args, level, reply}, t);
          } catch (e) {
