  ...
  async run ({message, args, level, reply}, t) { // eslint-disable-line no-unused-vars
    Settings.findOne({
      ...
      if (args[0].toLowerCase() === "list") {
        ...
        if (!settings.tags[indexer]) return reply(`Tag with position **${args[1]}** dosen't exist. Type \`${settings.prefix}tags list\` to view the list of available tags an their positions for your server.`);
